...
public class ClassEx08_내정답 {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			System.out.print("좌(1) 우(2) : ");
			int move = scan.nextInt();
			
			if(move == 1) {
				if(player != 0 && e.game[player - 1] == 1) {
					System.out.print("공격(3) : ");
					int atk = scan.nextInt();
					if(atk == 3) {
						e.game[player-1] = 2;
						e.game[player] = 0;
						player --;
					}else if(atk != 3){continue;}
				}else if(player != 0 && e.game[player - 1] == 0) {
					e.game[player - 1] = 2;
					e.game[player] = 0;
					player --;
				...
				if(player != 10 && e.game[player + 1] == 1) {
					System.out.print("공격(3) : ");
					int atk = scan.nextInt();
					if(atk == 3) {
						e.game[player+1] = 2;
				...
				}else if(player != 10 && e.game[player + 1] == 0) {
					e.game[player + 1] = 2;
					e.game[player] = 0;
					player ++;
